...
def run_doctor(settings: Settings) -> list[Check]:
    ...
    try:
        candles = client.public_candles(settings.inst_id, settings.bar, limit=5)
        checks.append(Check("okx_public_market", len(candles) > 0, f"{len(candles)} candles"))
    except Exception as exc:
        checks.append(Check("okx_public_market", False, str(exc)))
    try:
        instrument = client.public_instrument(settings.inst_id)
        detail = f"instType={instrument.get('instType')} ctVal={instrument.get('ctVal')} lotSz={instrument.get('lotSz')}"
        checks.append(Check("okx_instrument", bool(instrument.get("instId")), detail))
    except Exception as exc:
    ...
    if settings.okx_api_key and settings.okx_api_secret and settings.okx_api_passphrase:
        try:
            ...
            checks.append(Check("okx_private_account", equity is not None, f"USDT equity={equity}"))
        except Exception as exc:
