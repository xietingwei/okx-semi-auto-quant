...
[![测试](https://img.shields.io/badge/tests-51%20passing-2ea44f)](#测试)
...
**QIS** 是一个本地优先、可解释的 OKX 现货与永续市场研究和交易决策辅助系统。它将实时行情、多套预测策略、前向验证、持仓级风险控制和可选的大模型决策小精灵整合在同一套工作流中。
...
| 能力 | 说明 |
| --- | --- |
| 机会雷达 | 使用经过校准的多周期预测，对加密货币、外部美股日线和可选 OKX 股票映射标的进行排序 |
| 策略工作台 | 对比综合自适应、趋势跟随、突破确认和均值回归策略 |
| 实时价格预测 | 基于 OKX 最新成交价重新计算特征、概率、收益和目标价 |
| 市场环境 | 综合盘口深度、资金费率、持仓量、成交量结构、宏观数据和市场广度 |
| 深度分析 | 基于最长 6 个月日 K，逐日结合量化事实和外部消息生成原因推测，经过后续走势验证后汇总为超级大脑模式库 |
...
| --- | --- | --- | --- | --- |
| 综合自适应 | 全周期 | 以趋势为主，并结合拥挤度和市场环境 | 混合行情或环境切换期 | 极端冲击下多个因子同时失效 |
| 趋势跟随 | 1–6 个月 | 跟随 30/90 日主导趋势 | 持续单边行情 | 震荡行情反复止损 |
| 突破确认 | 1 天–1 个月 | 结合成交量、盘口和持仓确认动量突破 | 盘整后的波动扩张 | 假突破 |
| 均值回归 | 1 天–1 个月 | 对统计意义上的过度偏离进行反向交易 | 区间震荡和冲击后修复 | 强趋势中逆势接刀 |
...
校准采用近期样本权重更高、带边界约束的收缩方法。它可以降低置信度或将预测拉回中性，但跨市场校准不能反转单个标的的原始方向。
